...
void runScanner(std::vector<Signature>& signatures, std::vector<InputFile>& inputs)
{
	...
	*/
	...
	for(size_t i = 0; i < inputs.size(); i++)
	{
		results[i] = new bool[signatures.size()];
	}
	...
	for(size_t file_idx = 0; file_idx < inputs.size(); file_idx++)
	{
		...
		bool* dMatchArr;
		check_cuda_error(cudaMalloc((void **)&dMatchArr, signatures.size() * sizeof(bool)));
		check_cuda_error(cudaMemset(dMatchArr, 0, signatures.size() * sizeof(bool)));
		...
		for(size_t sig_idx = 0; sig_idx < signatures.size(); sig_idx++)
		{
			...
			matchFile<<<grid_size, BLOCK_SIZE, /* shared memory per block: */ 0, streams[file_idx]>>>(
				...
				sig_bufs[sig_idx], signatures[sig_idx].size, sig_idx, dMatchArr);
		}

		// copy the dMatchArr back to the host
